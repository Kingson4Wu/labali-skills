...
rg -n -- "--audio_file|--title|--description|--show_name|--season_number|--episode_number|--confirm_publish|--disable_d1|--cdp_port|--show_home_url" "$RUNNER" >/dev/null
...
rg -n "auto-executor|D1|D2|downgrade" "$SKILL_MD" "$ARCH" "$PLAN" >/dev/null
