...
- `show_home_url`: direct show home page URL (helps the executor locate the show faster)
- `disable_deterministic_cache`: `true` to skip deterministic cache and run policy executor directly
- `profile_dir`: browser profile directory for manual-login session reuse
- `cdp_port`: attach to an existing Chrome session (for example `9222`)
...
- Required fields are still: `audio_file`, `title`, `description`, `show_name`
