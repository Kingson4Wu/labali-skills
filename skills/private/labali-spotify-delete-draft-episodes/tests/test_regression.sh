...
# Ensure delete and verification guards are present.
rg -n "verifyDraftEmpty|deleteAllDraftEpisodes|ensureDraftFilter|delete-first|delete-all" "$SCRIPTS_DIR" >/dev/null
rg -n -- "--show_id|--delete_all_drafts|--max_delete|--disable_deterministic_cache|--cdp_port|--show_home_url" "$RUNNER" >/dev/null
rg -n -- "--show_id|--delete_all_drafts|--max_delete|--cdp_port" "$DET_RUNNER" >/dev/null

# Ensure required inputs are documented.
rg -n "show_id|delete_all_drafts|max_delete|disable_deterministic_cache|cdp_port|show_home_url" "$SKILL_YAML" >/dev/null

# Enforce layer boundary docs.
rg -n "Layer Contract|Success Criteria|Operational Mode" "$SKILL_MD" >/dev/null
rg -n "Layered Boundaries|Execution Model|Delete Correctness Standards" "$ARCH" >/dev/null
